    ...
    UIMainScene.IUIInfoContent
{
    ...
    //void Update() //Test function to see if the Unit is updating correctly
    //{
    //    Debug.Log($"{gameObject.name} running Unit.Update()");
    //    if (m_Target != null && Vector3.Distance(transform.position, m_Target.transform.position) < 2f)
    //    {
    //        BuildingInRange();
    //    }
    //}
    ...
    private void Update()
    {
        if (m_Target != null)
        {
            //Debug.Log($"Target type: {m_Target.GetType()}");
            float distance = Vector3.Distance(m_Target.transform.position, transform.position);
            //Debug.Log($"Distance to target: {distance}");
            if (distance < 2.0f)
            {
                m_Agent.isStopped = true;
                //Debug.Log("1: Calling BuildingInRange()");
                BuildingInRange();
                //Debug.Log($"{GetName()} is in range of {m_Target.GetName()}");
            }
        }
    }
